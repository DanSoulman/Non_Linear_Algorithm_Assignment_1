...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	//-------------------------------------------------------------------
	// 5. Basic Operation --> Get right tree of myBinarySearchTree: my_right_tree
	//-------------------------------------------------------------------
	...
	public int my_count_at_level(int level){
		...
		//DONE
		int res = 0;
		//1.1 If the tree is empty, return 0. Theres nothing
		if(my_is_empty() == true ){
			return 0;
		}
		//1.2 If level count is at 0 We've found a node.
		else if(level == 0){
			return 1;
		}
		//1.3 Counts down (decrements) level to the level we want.
		else{
			//1.3.1 Counts the given node on the left subtree. 
			res += this.my_left_tree().my_count_at_level(level-1);
			//1.3.2 Counts the given node on the right subtree.
			res += this.my_right_tree().my_count_at_level(level-1);
			
		}
		//2. We return res, our value
		return res;
	}
	...
	public boolean my_is_balanced(){
		...
		//1.Checks if the right tree isn't empty and gets it's length
		...
		//2.Checks if the left tree isn't empty and gets it's length
		...
		//3. Checks if balanced initially 
		if(leftLength+1 == rightLength || leftLength == rightLength ||  leftLength == rightLength+1){
			boolean rightBalanced = true; 	//Keeps track of if its balanced
			//4. Recursively checks right hand side
			if(rightLength>0) {
				rightBalanced = this.my_right_tree().my_is_balanced();
			}
			boolean leftBalanced = true;	//Keeps track of if its balanced
			//5. Recursively checks left hand side
			if(leftLength>0) {
				leftBalanced = this.my_left_tree().my_is_balanced();
			}
			//6.1 If both are still balanced after check
			if(rightBalanced && leftBalanced){
				return true;
			}
			//6.2 If they aren't balanced it returns false
			else{
				return false;
			}
		}
		//3.2 If Not balanced it's false
		else{
			return false;
		}
	}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		//2.1 We compare the root element to it, If less than it falls in here
		if (this.my_root().getKey().compareTo(key) == -1) {
			//2.1.1 Add the count on the left tree using the previously exisiting node count function  
			res += my_left_tree().my_node_count() + 1;
			//2.1.2 If there are nodes still on the Right Tree add them by recursively checking the tree.
			if (this.my_root().getRight() != null) {
				res += this.my_right_tree().my_count_smaller_nodes(key);
			}
		//2.2 Otherwise if key was equal to or bigger
		} else if (this.my_root().getKey().compareTo(key) >= 0) {
			//2.2.1 If there is are nodes still on the Left add them by checking recursively 
	...
	public int my_find_node_at_level(T1 key){
		//DONE
		int res = 0; // Return value
		//If list is empty We dont have the node in the tree and return -1 as we must return an int
		if(my_is_empty() == true){
			System.out.println("\tNode " +key+" does not exist in tree below.");
			return -1;
		}
		//If the key is the same as the first node (in our case 8) it is the top level. (Which I assume is 0)
		else if(this.root.getKey().compareTo(key) == 0){
			return 0;
		}
		//If the number is less than the key we recursively check left
		else if(this.root.getKey().compareTo(key) == -1){
			res = my_right_tree().my_find_node_at_level(key);
		}
		//If the number is greater than they key we recursively check right
		else if(this.root.getKey().compareTo(key) == +1){
			res = my_left_tree().my_find_node_at_level(key);
		}
		//Increments on each loop to find the correct level (e.g if it wasn't on 0 it increments to level 1)
		if(res!=-1){
			res++;
		}
		return res;
